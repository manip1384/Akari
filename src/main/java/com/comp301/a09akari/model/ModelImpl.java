...
public abstract class ModelImpl implements Model {
    ...
    public boolean isSolved() {
        ...
        for (int r = 0; r < puzzle.getHeight(); r++) {
            for (int c = 0; c < puzzle.getWidth(); c++) {
                ...
                if (puzzle.getCellType(r, c) == CellType.CLUE && !isLampPlacementValid(r, c)) {
                    return false;
                }
            }
        }
    ...
    private boolean isLampPlacementValid(int r, int c) {
        if (puzzleLibrary.getPuzzle(activePuzzleIndex).getCellType(r, c) == CellType.CLUE) {
            return false;
        }
        for (int[] lamp : lampLocations) {
            int lampR = lamp[0];
